...
export class DaybookReportComponent implements OnInit {
  ...
  loadMasterData() {
    this.adminService.getFinancialYears().subscribe((data: any) => {
      ...
      console.log('DP Processed Year:', this.financialYears[0]);
    });
    this.adminService.getReportGroups().subscribe((data: any) => {
      ...
      console.log('DP Processed Group:', this.groups[0]);
    });
    this.adminService.getReportTypes().subscribe((data: any) => {
      ...
      console.log('DP Processed Type:', this.types[0]);
    });
    this.adminService.getGroupLedgerMaster().subscribe((data: any) => {
      ...
      console.log('DP Processed Ledger:', this.ledgers[0]);
    });
  }

  onDisplay() {
    if (this.filterForm.valid) {
      this.adminService.getDaybookReport(this.filterForm.value).subscribe({
        next: (response: any) => {
          ...
          if (this.reportData.length === 0) {
            console.log('No data returned for filters:', this.filterForm.value, 'Response:', response);
          }
        },
        error: (err) => {
          console.error('Failed to load daybook data', err);
          alert('Failed to load daybook data: ' + (err.error?.message || err.message || 'Check console for details'));
        }
      });
    } else {
  ...
  onGeneratePdf() {
    if (this.filterForm.valid) {
      this.adminService.generateDaybookReport(this.filterForm.value).subscribe({
        next: (blob: any) => {
